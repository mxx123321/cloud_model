...
def calMetric_somemetric(predict, label):
    tp = np.sum(np.logical_and(predict == 1, label == 1))
    tn = np.sum(np.logical_and(predict == 0, label == 0)) #true positive 真的负样本
    fp = np.sum(np.logical_and(predict == 1, label == 0)) #flase positive 假的正样本 负样本 分为真的
    fn = np.sum(np.logical_and(predict == 0, label == 1)) #false negative 假的负样本 真的样本  预测为假
    return tp,tn,fp,fn #calMetric_somemetric
...
# 保存图像文件
#/home/liwei/zwq/code_c/fuse_withloss_No_redundancy/test_vis
        #cv2.imwrite("/home/liwei/zwq/code_c/fuse_withloss_No_redundancy/test_vis/before.png", label)
        label = make_one_hot(label.unsqueeze(0).long(), 2).squeeze(0)
        #cv2.imwrite("/home/liwei/zwq/code_c/fuse_withloss_No_redundancy/test_vis/after.png", label)
...
class TestDatasetFromFolder(Dataset):
    def __init__(self, args, Time1_dir, Label_dir):
    ...
    def __getitem__(self, index):
        ...
        return image1, label, image_name
...
class DA_DatasetFromFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(label,"lalbel lalbel lalbel lalbel")
        ...
        #cv2.imwrite("/home/liwei/zwq/code_c/fuse_withloss_No_redundancy/test_vis/before.png", array)
        label = make_one_hot(label.unsqueeze(0).long(), 2).squeeze(0)
        #array = label.cpu().numpy()
        #cv2.imwrite("/home/liwei/zwq/code_c/fuse_withloss_No_redundancy/test_vis/after.png", array)
        #print(label.shape,"lalbel lalbel lalbel lalbel")
